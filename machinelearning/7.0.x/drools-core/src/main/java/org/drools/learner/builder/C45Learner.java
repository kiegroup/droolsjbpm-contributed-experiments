...
public class C45Learner extends Learner {
    ...
    protected TreeNode train( DecisionTree dt, InstDistribution data_stats, int depth ) {//List<Instance> data) {
        ...
        if ( attribute_domains.size() == 0 ) {
            ...
            Object winner = data_stats
                    .get_winner_class(); /* winner target category */
            LeafNode noAttributeLeftNode = new LeafNode(
                    dt.getTargetDomain() /* target domain */, winner );
            noAttributeLeftNode.setRank( data_stats.getVoteFor( winner ) / (double) this
                    .getTrainingDataSize() /* total size of data fed to dt */ );
        ...
        if ( super.criteria != null && criteria.size() > 0 ) {
            for ( StoppingCriterion sc : criteria )
                if ( sc.stop( best_attr_eval ) ) {
                    ...
                    majorityNode.setRank( (double) data_stats.getVoteFor( winner ) / (double) this
                            .getTrainingDataSize() /*
        ...
        currentNode.setRank( data_stats.getSum() / (double) this
