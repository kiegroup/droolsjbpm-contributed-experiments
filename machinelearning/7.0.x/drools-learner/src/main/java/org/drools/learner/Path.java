...
public class Path {
    ...
    /*
     * action nodeValue has the information of private double rank; // matching
     * ratio private double num_classified_instances;// number of instances
     * matching that rule private double info_mea;
     */
    ...
    /*
     * WHAT TODO ??? public static Comparator<Path> getInfoComparator() { return
     * new PathInfoComparator(); }
     * 
     * private static class PathInfoComparator implements Comparator<Path>{ //
     * this will sort from best rank to least rank public int compare(Path p1,
     * Path p2) { // COMPLEXITY++; // if (n1.hashCode() == n2.hashCode()) //
     * return 0; double p1_mea = p1.getInfoMea(); double p2_mea =
     * p2.getInfoMea(); // switch (INFO_MEA) { // case 4: /* 4 - ranked gain
     * ration/ // p1_mea = p1.getRank()*p1_mea; // p2_mea = p2.getRank()*p2_mea;
     * // break; // }
     * 
     * //if a node with the same domain exist at the same depth //COMPLEXITY++;
     * if (p1_mea < p2_mea) return 1; // inverted=>must be -1 //COMPLEXITY++; if
     * (p1_mea > p2_mea) return -1; // inverted=>must be 1 //COMPLEXITY++;
     * //else { return 0; } }
     */
    @Override
    public int hashCode() {
