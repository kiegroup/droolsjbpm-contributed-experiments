 ...
 * 
 * 
 * Instance: A single object of the world from which a model will be learned, or
 * on which a model will be used (e.g., for prediction). In most machine
 * learning work, instances are described by feature vectors; some work uses
 * more complex representations (e.g., containing relations between instances or
 * between parts of instances). = (example, case, record)
 * 
 * Schema: A description of a data set's attributes and their properties.
 * 
 * Attribute: A quantity describing an instance. An attribute has a domain
 * defined by the attribute type, which denotes the values that can be taken by
 * an attribute. = (field, variable, feature)
 * 
 * The following domain types are common: - Categorical: A finite number of
 * discrete values. The type nominal denotes that there is no ordering between
 * the values, such as last names and colors. The type ordinal denotes that
 * there is an ordering, such as in an attribute taking on the values low,
 * medium, or high.
 * 
 * - Continuous (quantitative): Commonly, subset of real numbers, where there is
 * a measurable difference between the possible values. Integers are usually
 * treated as continuous in practical problems.
 * 
 * Feature: A feature is the specification of an attribute and its value. For
 * example, color is an attribute. ``Color is blue'' is a feature of an example.
 * Many transformations to the attribute set leave the feature set unchanged
 * (for example, regrouping attribute values or transforming multi-valued
 * attributes to binary attributes). Some authors use feature as a synonym for
 * attribute (e.g., in feature-subset selection).
 * 
 * Feature vector: A list of features describing an instance. = (record, tuple)
 * 
 */