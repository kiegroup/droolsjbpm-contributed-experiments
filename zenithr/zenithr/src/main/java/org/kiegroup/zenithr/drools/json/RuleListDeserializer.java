...
public class RuleListDeserializer extends JsonDeserializer<List<Rule>> {
    ...
    @Override
    public List<Rule> deserialize(JsonParser p, DeserializationContext ctxt) throws IOException, JsonProcessingException {
        JsonNode nodes = p.getCodec().readTree(p);
        ObjectMapper mapper = new ObjectMapper();
        List<Rule> rules = new ArrayList<>();
        for(int i = 0; i < nodes.size(); i++) {
            JsonNode node = nodes.get(i);
            rules.add(mapper.readValue(node.toString(),  Rule.class).setId(i));
        }
        return rules;       
    }
